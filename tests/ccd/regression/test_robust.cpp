...
{
    index_t band;
    std::array<scalar_t, 5> coefficients;
    index_t nonzero;
    scalar_t rmse;
    std::vector<scalar_t> residuals;
};

std::vector<RobustReference> load_reference(const std::string& filename)
{
    ...
    std::vector<RobustReference> refs;
...
TEST(RobustSolver, MultiBandRegression)
{   
    constexpr index_t n_samples = 12;
    constexpr index_t n_bands   = 7;

    auto reference = load_reference("robust_reference.csv");
    ...
    {
        Eigen::Map<const Eigen::VectorXd> y_band(
            y[band].data(),
            n_samples
        );
        ...
        ASSERT_EQ(model.coefficients().size(), ref.coefficients.size());
