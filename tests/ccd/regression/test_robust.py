...
from ccd.models.tmask import tmask_coefficient_matrix
...
robust = robust_fit.RLM(maxiter=5)

rows = []

for band in range(7):
    model = robust.fit(X, y[band, :])
    predictions = model.predict(X)
    rmse, residuals = calc_rmse(y[band, :], predictions, num_pm=5)
...
df.to_csv("robust_reference.csv", index=False)

print("\nSaved results to robust_reference.csv")
