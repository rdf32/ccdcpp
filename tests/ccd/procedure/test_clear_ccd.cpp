...
inline void print_change_model(const ccd::CCDChangeModel& model)
{
    std::cout << "\n========================================\n";
...
inline void print_change_models(
    const std::vector<ccd::CCDChangeModel>& models
)
{
    for(std::size_t i = 0;
        i < models.size();
...
TEST(CCD, DetectTest)
{
    ...
    ccd::CCDLassoOptions loptions;
    ccd::CCDLassoSolver solver(loptions);

    ccd::CCDLassoWorkspace lworkspace(data.dates.size());

    ccd::TestCCD fit_procedure;
    ccd::CCDFitResult result = fit_procedure.run(
        hworkspace,
        lworkspace,
        solver
    );
