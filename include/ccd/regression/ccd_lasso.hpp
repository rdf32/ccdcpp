...
// for fit every band can reuse the same X, X_mean, X_cent, column_norm2 -> LassoBasis
// for predict every band can reuse the same X 

// std::vector<scalar_t> X_center;
...
struct CCDLassoWorkspace
{
    ...
    explicit CCDLassoWorkspace(index_t max_samples) // build outside loop
    {
